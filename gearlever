...
BINSAVED="certificates uname cat readelf readlink dirname grep" # Enter here keywords to find and save in /usr/bin
...
LIBSAVED="pk p11 alsa jack pipewire pulse girepository Graphene cairo libgstplayer libcups \
	...
	gdk-pixbuf librsvg libdav libLLVM libGL libgdk-3 libgtk-3 libOpenGL $lib_browser_launcher" # Enter here keywords or file/directory names to save in /usr/lib
...
function _savebins() {
	...
	mv ./"$APP".AppDir/.junest/usr/bin/head ./save/
	mv ./"$APP".AppDir/.junest/usr/bin/id ./save/
	mv ./"$APP".AppDir/.junest/usr/bin/ls ./save/
	mv ./"$APP".AppDir/.junest/usr/bin/sed ./save/
