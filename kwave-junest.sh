...
DEPENDENCES="$(curl -Ls "https://gitlab.archlinux.org/archlinux/packaging/packages/kwave/-/raw/main/.SRCINFO" | grep "depends =" | grep -v makedepends | awk '{print $3}' | sed 's/://g' | sort -u | xargs)"; [ -z "$DEPENDENCES" ] && exit 0; DEPENDENCES="$DEPENDENCES pipewire"
...
SHARESAVED="alsa"
LIBSAVED="alsa jack libogg.so libvorbis libFLAC.so libopus.so libmpg libmp3lame.so pipewire pulse"
