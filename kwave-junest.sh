...
LIBSAVED="libproxy libnghttp libidn libssh libpsl.so libunistring.so libxcb libuuid libGLX \
libogg.so libvorbis libFLAC.so libopus.so libmpg libmp3lame.so libLLVM $lib_audio_keywords $lib_browser_launcher"
...
mv "$APP".AppDir/*.svgz "$APP".AppDir/"$APP".svg
