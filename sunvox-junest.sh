...
lib_audio_keywords="alsa jack libogg.so libvorbis libFLAC.so libopus.so libmpg libmp3lame.so pipewire pulse"
lib_browser_launcher="gio-launch-desktop libasound.so libatk-bridge libatspi libcloudproviders libdb- libdl.so libedit libepoxy libgtk-3.so.0 libjson-glib libnssutil libpthread.so librt.so libtinysparql libwayland-cursor libX11-xcb.so libxapp-gtk3-module.so libXcursor libXdamage libXi.so libxkbfile.so libXrandr p11 pk"
LIBSAVED="libSDL $lib_audio_keywords $lib_browser_launcher"
...
if [ -n "$APP" ]; then
	...
	./.local/share/junest/bin/junest -- yay --noconfirm -S --skippgpcheck "$APP"
