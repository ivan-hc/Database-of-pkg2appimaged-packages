...
DEPENDENCES="libvlc" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
BASICSTUFF="binutils debugedit gzip jack2 qt6-multimedia"
...
LIBSAVED="libproxy libxcb qt libuuid libpulsecommon libjack.so Multimedia libGLX \
libLLVM libsensors.so libxshmfence.so libdrm libsodium.so libsoxr.so libglslang-default-resource-limits.so vlc vdpau libSDL $lib_audio_keywords $lib_browser_launcher"
...
_extract_package() {
	...
	if test -f "$pkg_full_path"; then
		...
		[ -n "$lib_browser_launcher" ] && [[ "$arg" =~ (hicolor-icon-theme|xapp|v4l-utils|gstreamer|ffmpeg) ]] && tar fx "$pkg_full_path" -C ./base/ --warning=no-unknown-keyword --exclude='.PKGINFO'
	fi
}
