...
LIBSAVED="cairo girepository libadwaita libharfbuzz libavahi-common.so libgstplay libavahi-client.so libcurl.so \
libgnutls.so libgstbase libxmlb.so liblcms liborc libyaml libudev.so libunwind.so libEGL.so libGLX.so libgudev \
libdrm.so libgbm.so urllib idna gdk-pixbuf libdav libseccomp.so libdebuginfod libjson \
libOpenGL.so libxslt.so $lib_audio_keywords $lib_browser_launcher"
...
bind_files="/etc/resolv.conf /etc/hosts /etc/nsswitch.conf /etc/passwd /etc/group /etc/machine-id /etc/asound.conf /etc/localtime /etc/os-release "
...
_extract_package() {
	...
	if test -f "$pkg_full_path"; then
		...
		[ -n "$lib_browser_launcher" ] && [[ "$arg" =~ (hicolor-icon-theme|xapp|7zip|libibus|gdk-pixbuf2|libadwaita|python-dbus|python-graphene|python-chardet) ]] && tar fx "$pkg_full_path" -C ./base/ --warning=no-unknown-keyword --exclude='.PKGINFO'
	fi
}
